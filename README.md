...
## 測試金流 (Stripe Test Payment)

在開發或測試環境下結帳時，請使用以下的信用卡號進行模擬付款：

- **信用卡號 (Card Number)**: `4242 4242 4242 4242`
- **有效期限 (MM/YY)**: 任何未來的日期皆可（例如：`12/30`）
- **安全碼 (CVC/CVV)**: 任何 3 位數字（例如：`123`）
- **郵遞區號 (ZIP)**: 任何 5 位數字（例如：`12345`）
