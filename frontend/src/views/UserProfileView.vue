  ...
  <div class="min-h-screen bg-gray-50 pt-24 pb-12">
    ...
    <div v-else-if="user" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex justify-between items-start">
              <div class="w-full">
                <div v-if="isEditing">
                  ...
                  <input
                    ...
                    type="text"
                    class="mb-4 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border p-2"
                  />
                ...
                <div v-else>
                  ...
                  <p
                    v-if="user.bio"
                    class="text-gray-600 text-lg leading-relaxed whitespace-pre-wrap"
                  >
                    {{ user.bio }}
                  </p>
                  <div v-else class="text-gray-400 italic">
                    ...
                    <button
                      v-else
                      @click="startEdit"
                      class="text-indigo-600 hover:text-indigo-500 font-medium underline decoration-dashed underline-offset-4"
                    >
                      {{ profile.create }} &rarr;
                    </button>
                  </div>
              ...
              <div v-if="isCurrentUser" class="ml-4 flex-shrink-0">
                <div v-if="isEditing" class="flex gap-2">
                  <button
                    @click="cancelEdit"
                    class="flex items-center gap-1 px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors"
                  >
                    <X class="w-4 h-4" /> {{ profile.cancel }}
                  </button>
                  <button
                    @click="saveProfile"
                    class="flex items-center gap-1 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors shadow-sm"
                  >
                ...
                <button
                  v-else-if="user.bio"
                  @click="startEdit"
                  class="flex items-center gap-1 px-4 py-2 border border-indigo-200 text-indigo-700 rounded-lg hover:bg-indigo-50 transition-colors"
                >
      ...
      <h2 class="text-2xl font-bold text-gray-900 mb-6">
        {{ profile.user_products.replace("{name}", user.name) }}
      </h2>
    ...
    <div v-else class="text-center py-20">
      ...
      <router-link
        to="/"
        class="text-indigo-600 hover:text-indigo-500 mt-4 inline-block"
        >{{ profile.return_home }}</router-link
      >
    </div>
  </div>
</template>
